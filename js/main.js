...
var GoogleAPIMailClient = window.GoogleAPIMailClient || (function () {
        ...
        function createTags() {
            var oReg = GoogleAPIMailClient.oRegexes;
            var aItems = [];

            for (var key in oReg) {
                // skip loop if the property is from prototype
                if (!oReg.hasOwnProperty(key)) continue;
                ...
                for (var prop in obj) {
                    ...
                    // create array from suppliers' names in oRegexes
                    if (aItems === undefined || aItems.length == 0) {
                        aItems = obj[prop]['name'];
                    } else {
                        var aSubItems = obj[prop]['name'];
                        Array.prototype.push.apply(aItems, aSubItems);
                        aSubItems = [];
                    }
                }
            }

            // add tags to query input
            $('#q').tagEditor({
                autocomplete: {
                    delay: 0,
        ...
        function parseMessage(message, i) {
            ...
            //console.log('     ');
            //console.log('query = ' + aUSER_QUERY[i]);
            //console.log('i = ' + i + '   MESSAGE ID = ' + message['id']);
